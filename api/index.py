...
def save_all_sessions():
    ...
    if not os.access(os.path.dirname(SESSION_FILE), os.W_OK):
        print(f"[*] WARNING: {SESSION_FILE} is not writable (expected on Vercel).")
        return
    try:
...
async def chat(request: ChatSchema, x_gemini_key: str = Header(None, alias="X-Gemini-Key"), x_user_id: str = Header(None, alias="X-User-ID")):
    ...
    try:
        ...
        if response.get("images"):
            print(f"[*] DEBUG: Detected {len(response['images'])} images in response.")
